...
<!-- This is a php program which allows users to see posts made by all users. -->
    ...
    <?php
        ...
        if ($_SERVER["REQUEST_METHOD"] === "POST" &&
            ...
            !empty($_SESSION['uid'])
                ...
                header("Location: feed.php"); // Send user to feed.php (basicly refresh)
                exit();
        }
        ...
        foreach ($posts as $pid){ // Takes posts array and makes a new variable pid for every iteration in the loop.
            ...
            if (!empty($_SESSION['uid']) && !empty($_SESSION['password'])) {
                if($_SERVER["REQUEST_METHOD"] == "POST"){
                    ...
                    header("Location: feed.php"); // Sends user to feed.php. Basicly reloads page.
                    exit();
                }
